 ...
 *   --help                Show help
 */
import { readFileSync, readdirSync, writeFileSync, existsSync, statSync, mkdirSync, unlinkSync, } from "node:fs";
...
// Code generation
// ---------------------------------------------------------------------------
function generateServerFile(files, outputPath, componentApiName) {
    const textEntries = [];
    const binaryEntries = [];
...
// ---------------------------------------------------------------------------
async function uploadModulesTarball(standaloneDir, componentName, deploymentId) {
    const modulesDir = join(standaloneDir, "node_modules");
    if (!existsSync(modulesDir)) {
        throw new Error(`Standalone node_modules not found at ${modulesDir}`);
    }
    // Create tarball of the standalone node_modules
    const tarballPath = join(standaloneDir, "__next_modules__.tar.gz");
    execSync(`tar czf "${tarballPath}" -C "${standaloneDir}" node_modules`, { stdio: "pipe" });
    const tarballSize = statSync(tarballPath).size;
    console.log(`  Tarball: ${(tarballSize / 1024 / 1024).toFixed(1)} MiB`);
...
function ensureConvexJson(convexDir) {
    ...
    const externalPackages = node.externalPackages || [];
...
async function main() {
    ...
    if (!args.skipUpload) {
        ...
        try {
            ...
            console.error("  npx @convex-dev/self-hosting next-build --skip-build --prod");
        }
